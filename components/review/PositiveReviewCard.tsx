...
export function PositiveReviewCard({googleUrl,websiteUrl}:{googleUrl:string;websiteUrl:string}) {
 return <section className="response-card positive reveal" aria-labelledby="positive-title"><div className="mood-icon"><Smile size={33} strokeWidth={1.5} aria-hidden="true"/></div><h2 id="positive-title">Thank You!</h2><p>We’re glad you enjoyed your experience at BiBiChik SS2.</p><GoogleReviewLink url={googleUrl} primary/>{websiteUrl && <a className="website-link" href={websiteUrl}>Continue to BiBiChik Website</a>}</section>;
}
