...
export function ReviewPage({outlet}:{outlet:Outlet}) {
 ...
 useEffect(()=>{
  ...
  try{Promise.resolve(context.registerTool({name:"select_experience_rating",description:"Select a 1–5 star rating. Ratings 1–3 open the feedback form. Ratings 4–5 show the thank-you card and this outlet's Google Review link when configured. Does not submit feedback or navigate.",inputSchema:{type:"object",properties:{rating:{type:"integer",minimum:1,maximum:5}},required:["rating"],additionalProperties:false},annotations:{readOnlyHint:false},execute:(input:unknown)=>{
 ...
 return <div className="site-shell" data-theme={outlet.theme}><main className="review-container">
  ...
  {rating>=4 && <PositiveReviewCard googleUrl={googleUrl} websiteUrl={websiteUrl} outletName={outlet.outletName} brand={outlet.brand}/>}
